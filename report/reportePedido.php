...
function Header()
{
    ...
    $this->setXY(60,60);
    $this->Cell (100,10,utf8_decode('Reporte de Pedidos HM'),0,1,'C');
    $this->Ln(10);
    //Fecha del Sistema
    ...
    $this->Cell (60,10,utf8_decode('Estado del Pedido'),1,0,'C',1);
    $this->Cell (35,10,utf8_decode('Tipo de entrega'),1,0,'C',1);
    $this->Cell (30,10,utf8_decode('N° Referencia'),1,0,'C',1);
    $this->Cell (35,10,utf8_decode('Total Ganancia'),1,0,'C',1);
    $this->Ln(10);
}
...
$suma=0;
$sDet=ejecutarSQL::consultar("SELECT * FROM venta");
while($fila1 = mysqli_fetch_array($sDet, MYSQLI_ASSOC)){
    $consulta=ejecutarSQL::consultar("SELECT * FROM venta");
    $fila=mysqli_fetch_array($consulta, MYSQLI_ASSOC);
    $pdf->Cell (30,10,utf8_decode($fila1['Fecha']),1,0,'C');
    $pdf->Cell (60,10,utf8_decode($fila1['Estado']),1,0,'C');
    $pdf->Cell (35,10,utf8_decode($fila1['TipoEnvio']),1,0,'C');
    $pdf->Cell (30,10,utf8_decode($fila1['NumeroDeposito']),1,0,'C');
    $pdf->Cell (35,10,utf8_decode($fila1['TotalPagar']),1,0,'C');
    $pdf->Ln(10);
