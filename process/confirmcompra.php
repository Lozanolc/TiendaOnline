...
if(mysqli_num_rows($verdata)>=1){
  ...
  if(!empty($_SESSION['carro'])){
    ...
    if(consultasSQL::InsertSQL("venta", "Fecha, NIT, TotalPagar, Estado, NumeroDeposito, TipoEnvio, Adjunto", "'".date('d-m-Y')."','$Cedclien','$suma','$StatusV','$NumDepo','$tipoenvio','$comprobanteF'")){
      ...
      $verificar=ejecutarSQL::consultar("SELECT * FROM venta WHERE NumeroDeposito='".$NumDepo."'");
      if(mysqli_num_rows($verificar)<=0){
       if(consultasSQL::InsertSQL("venta", "NumeroDeposito", "'$NumDepo'")){
        echo '<script>
            swal({
              title: "La referencia fue validada con exito",
              type: "success",
       ...
       echo '<script>swal("ERROR", "El numero de referencia ya fue registrado", "error");</script>';
      }
      /*echo '<script>
      swal({
    ...
    echo '<script>swal("ERROR", "La cedula es incorrecta, no esta registrado con ningun cliente", "error");</script>';
}
