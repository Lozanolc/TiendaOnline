...
if(!$nitCliente=="" && !$nameCliente=="" && !$apeCliente=="" && !$dirCliente=="" && !$phoneCliente=="" && !$emailCliente=="" && !$fullnameCliente==""){
    if($passCliente==$passCliente2){
        ...
        $verificar1= ejecutarSQL::consultar("SELECT * FROM cliente WHERE Email='".$emailCliente."'");
        $verificaltotal = mysqli_num_rows($verificar);
        $verificaltotal1 = mysqli_num_rows($verificar1);
        if($verificaltotal<=0 && $verificaltotal1<=0){
        ...
        echo '<script>swal("ERROR", "Algunos datos (Cedula o Email) ya están registrados en sistema", "error");</script>';
       }
        mysqli_free_result($verificar);
        mysqli_free_result($verificar1);
        }else{
