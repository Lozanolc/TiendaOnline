<?php
	...
	if($oldUser!=$user){
		...
		if(mysqli_num_rows($SelectUser)==1){
			echo '<script>swal("El nombre de usuario que ha ingresado ya se encuentra registrado en el sistema, por favor escriba otro", "error");</script>';
			exit();
		}
	}
	...
	if($oldPass!="" && $newPass!="" && $newPass2!=""){
		if($newPass!=$newPass2){
			echo '<script>swal("Las contraseñas que acaba de ingresar no coinciden", "error");</script>';
			exit();
		}else{
			$oldPass=md5($oldPass);
			...
			if(mysqli_num_rows($CheckLog)==1){
				...
				echo '<script>swal("La contraseña actual no coincide con la que se encuentra registrada en el sistema", "error");</script>';
				exit();
			}
		}
	}else{
